...
export class PlistParser implements IParser {
  ...
  parse(content: string): SpriteSheetConfig {
    try {
      Logger.debug('解析 PLIST 配置文件', `大小: ${content.length} 字符`)
      ...
      if (!frames || typeof frames !== 'object') {
        throw ErrorHandler.createError(ErrorType.INVALID_CONFIG, 'PLIST 格式错误：缺少 frames')
      }
      if (!metadata || typeof metadata !== 'object') {
        throw ErrorHandler.createError(ErrorType.INVALID_CONFIG, 'PLIST 格式错误：缺少 metadata')
      }
      ...
      Logger.debug('PLIST 配置文件解析成功', `子图数量: ${spriteFrames.length}`)
      return {
      ...
      Logger.error('PLIST 配置文件解析失败', error)
      ErrorHandler.handleError(error, 'PlistParser.parse')
